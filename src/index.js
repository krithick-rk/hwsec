...
import { runAnalysisPipeline } from './core/pipelineRunner.js';
import { ConsoleApp } from './core/console/consoleApp.js';
...
program.command('proceed')
  ...
  .action(async (analysisId, options) => {
    ...
    try {
        ...
        const res = await runAnalysisPipeline(analysisId, options);
        console.log(`\n[+] Analysis execution COMPLETED successfully!`);
        console.log(`    DETECTED:           ${res.results?.detected_count || 0}`);
        console.log(`    NOT_DETECTED:       ${res.results?.not_detected_count || 0}`);
        console.log(`    INCONCLUSIVE:       ${res.results?.inconclusive_count || 0}`);
...
// ==========================================
// Command: console (Metasploit-Style Operations Console)
// ==========================================
program.command('console')
  .description('Launch the interactive HWSEC security operations console')
  .option('-c, --config <path>', 'Path to config file', 'config.json')
  .option('-o, --output-dir <path>', 'Base output directory', 'hwsec-output')
  .option('--no-onboarding', 'Skip first-run interactive onboarding wizard')
  .action(async (options) => {
    const consoleApp = new ConsoleApp({
        config: options.config,
        outputDir: options.outputDir,
        skipOnboarding: !options.onboarding
    });
